...
function PwaBanner() {
    ...
    useEffect(() => {
        if (!auth?.user) return;
        try { if (localStorage.getItem('pwa_descartado')) return; } catch {}
        ...
        // Ya estaba capturado antes de que React montara
        mostrar();

        // Por si llega después
        const handler = (e) => {
            e.preventDefault();
    ...
    const descartar = () => {
        try { localStorage.setItem('pwa_descartado', '1'); } catch {}
        setVisible(false);
    };
    ...
    return (
        <div className="fixed top-20 left-4 right-4 z-[60] bg-gray-900 border border-orange-500/50 rounded-2xl shadow-2xl p-4 flex items-center gap-3 sm:max-w-sm sm:left-auto sm:right-4">
            ...
            <div className="flex flex-col gap-1.5 shrink-0">
                <button onClick={instalar} className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors">Instalar</button>
                <button onClick={descartar} className="text-gray-500 hover:text-gray-300 text-xs text-center transition-colors">Ahora no</button>
            </div>
        </div>
    );
}
...
export default function TiendaLayout({ children }) {
    ...
    return (
        <div data-tema={temaClaro ? 'claro' : 'oscuro'}>
            ...
            <footer className="bg-gray-900 border-t border-gray-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-8">
                        ...
                        <div>
                            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Contáctanos</h4>
                            <ul className="space-y-3 text-sm text-gray-400">
                                <li>
                                    <a href="https://wa.me/573127675273?text=Hola%2C%20me%20interesa%20un%20producto%20de%20GadGet%20Store"
                                        target="_blank" rel="noopener noreferrer"
                                        className="flex items-center gap-2 hover:text-green-400 transition-colors">
                                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current text-green-400 shrink-0">
                                        ...
                                        WhatsApp · 312 767 5273
                                    </a>
                                </li>
                                <li>
                                    <a href="tel:+573127675273" className="flex items-center gap-2 hover:text-orange-400 transition-colors">
                                        <svg className="w-4 h-4 fill-current text-orange-400 shrink-0" viewBox="0 0 24 24">
                                            <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/>
                                        </svg>
                                        Celular · 312 767 5273
                                    </a>
                                </li>
                                <li>
                                    <a href="https://instagram.com/gadgetstoremed_2026" target="_blank" rel="noopener noreferrer"
                                        className="flex items-center gap-2 hover:text-pink-400 transition-colors group">
                                        <svg className="w-4 h-4 text-pink-400 shrink-0 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                            <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
                                            <circle cx="12" cy="12" r="4"/>
                                            <circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/>
                                        </svg>
                                        <span className="text-pink-400">@gadgetstoremed_2026</span>
                                    </a>
                                </li>
                                <li className="pt-1">📦 Envíos a toda Colombia</li>
                    ...
                    <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                            ...
                            { icon: '🚚', text: 'Envío a todo Colombia' },
                            { icon: '↩️', text: 'Devolución fácil' },
                            { icon: '⚡', text: 'Atención inmediata' },
                        ].map(({ icon, text }) => (
                            <div key={text} className="flex items-center gap-2 bg-gray-800/60 rounded-xl px-3 py-2.5 border border-gray-700/50 hover:border-orange-500/30 transition-colors">
            ...
            <a href="tel:+573127675273" title="Llámanos"
